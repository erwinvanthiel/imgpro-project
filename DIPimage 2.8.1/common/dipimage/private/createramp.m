   ...
   if length(sz)>=dim & sz(dim)>1
      ...
      switch origin
         ...
         case 'freq' % Use true frequency domain coordinates
            x = (-floor((x+1)/2):floor(x/2))/(x+1); % (zero to the right of the center)
         case 'radfreq' % Use radial frequency domain coordinates
            x = (-floor((x+1)/2):floor(x/2))*(2*pi/(x+1)); % (zero to the right of the center)
         ...
         case 'mfreq' % Use true frequency domain coordinates
            x = -(-floor((x+1)/2):floor(x/2))/x; % (zero to the right of the center)
         case {'mradfrq','mradfreq'} % Use radial frequency domain coordinates
            x = -(-floor((x+1)/2):floor(x/2))*(2*pi/x); % (zero to the right of the center)
         otherwise
